...
(function () {
    ...
    const SmartPrompt = {
        ...
        generateSmartPrompt() {
         // 1. 功能选择（处理 == 格式） - 添加复选框控制
const gongnengValue = document.getElementById('gongneng').value;
let gongnengItem = "";
const useGongneng = document.getElementById('gongnengCheckbox').checked; // 获取复选框状态

if (useGongneng && gongnengValue) { // 只有在复选框选中时才处理
    if (gongnengValue.includes("==")) {
        const parts = gongnengValue.split("==");
        if (parts.length >= 2) {
            gongnengItem = parts[1].trim();  // 使用 == 后面的部分
        } else {
            gongnengItem = gongnengValue;
        }
    } else {
        gongnengItem = gongnengValue;
    }
...
const otherRequiredItems = Array.from(
 ...
 let prompt = "";
  // 新顺序：gongneng,空间，空间必要，可选，附加，其它要求的可选，必选，灯光，风格
const allItems = [];

// 1. 功能选择 (gongneng)
if (gongnengItem) {
    allItems.push(gongnengItem);
}
...
// 6. 其它要求的可选
if (otherOptionalItems.length > 0) {
...
if (useLightingStyle && lightingStyle) { 
    // 处理 lightingStyle 的 == 格式
    let lightingStyleValue = lightingStyle;
    if (lightingStyle && lightingStyle.includes("==")) {
        const parts = lightingStyle.split("==");
        if (parts.length >= 2) {
            lightingStyleValue = parts[1].trim();
        }
    }
    allItems.push(lightingStyleValue);
}

// 9. 设计风格
if (useDesignStyle && designStyle) { 
    // 处理 designStyle 的 == 格式
    let designStyleValue = designStyle;
    if (designStyle && designStyle.includes("==")) {
        const parts = designStyle.split("==");
        if (parts.length >= 2) {
            designStyleValue = parts[1].trim();
        }
    }
    allItems.push(designStyleValue);
}
...
// 恢复 gongneng 复选框状态
if (appState.originalPromptConfig.useGongneng !== undefined) {
    document.getElementById('gongnengCheckbox').checked = appState.originalPromptConfig.useGongneng;
}
    ...
    const ColorPalette = {
       ...
       // 更新设计风格下拉菜单（使用与 gongneng 相同的处理逻辑）
...
const gongnengSelect = document.getElementById('gongneng');
if (gongnengSelect) {
    gongnengSelect.innerHTML = '';
    const gongnengList = furnitureConfig.gongneng || [];
    gongnengList.forEach(item => {
        const option = document.createElement('option');
        // 处理可能包含 == 分隔符的情况
        let displayValue = item;
        let displayText = item;
        ...
        option.value = displayValue;
        option.textContent = displayText;
        gongnengSelect.appendChild(option);
    });
}
    ...
    if (lightingStyleSelect) {
        ...
        lightingStyles.forEach(lighting => {
            const option = document.createElement('option');
            // 处理可能包含 == 分隔符的情况
            let displayValue = lighting;
            let displayText = lighting;
            
            if (lighting && lighting.includes("==")) {
                const parts = lighting.split("==");
                if (parts.length >= 2) {
                    displayText = parts[0];  // 显示 == 前面的部分
                    displayValue = lighting;     // 完整值
                }
            }
    ...
    const EventHandlers = {
        ...
        setupPromptEventListeners() {
            document.getElementById('gongnengCheckbox').addEventListener('change', () => SmartPrompt.generateSmartPrompt());
document.getElementById('gongneng').addEventListener('change', () => SmartPrompt.generateSmartPrompt());
